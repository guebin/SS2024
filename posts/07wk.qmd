...
$$B_1,\dots, B_n \in {\cal A} ~ \Longrightarrow ~ \tilde{m}\left(\biguplus_{i=1}^{n} B_{i}\right)=\sum_{i=1}^{n}\tilde{m}(B_i)$$
...
$$\text{(1)} B_1,B_2,\dots \in {\cal A} ~~\text{(2)}\biguplus_{i=1}^{\infty}B_i \in {\cal A}~ \Longrightarrow ~ \tilde{m}\left(\biguplus_{i=1}^{n} B_{i}\right)=\sum_{i=1}^{n}\tilde{m}(B_i)$$
...
$$A_1,\dots, A_n \in {\cal A} ~ \Longrightarrow ~ \tilde{m}\left(\bigcup_{i=1}^{\infty} A_{i}\right)\leq\sum_{i=1}^{n}\tilde{m}(A_i)$$
...
$$\text{(1)}A_1,A_2,\dots \in {\cal A} ~~\text{(2)}\bigcup_{i=1}^{\infty}A_i \in {\cal A}~ \Longrightarrow ~ \tilde{m}\left(\bigcup_{i=1}^{n} A_{i}\right)\leq\sum_{i=1}^{\infty}\tilde{m(A_i)}$$
...
`-` 위의 정의에서 ${\cal A}$ 시그마필드라면, $\sigma$-additive 와  $\sigma$-subadditive 에서 조건 (2) 은 생략할 수 있다. 
...
:::{.callout-note}
시그마유한 측도의 뉘앙스는 "**전체 집합을 유한한 값으로 나타낼 길은 없을지라도, 전체집합을 쪼개서 (비록 무한번 쪼개는 한이 있더라도), 쪼갠것들은 유한한 값으로 나타낼 수 있는 길이 최소한 하나는 존재한다.**" 의 뉘앙스를 가지고 있다. 따라서 시그마측도의 정의에서 집합열 $\Omega_1,\Omega_2,\dots$ 를 무한집합열로 잡을 이유는 굳이 없다. (수 틀리면 무한번 쪼개겠다는 것이지 꼭 무한번 쪼개야 할 이유는 없으니까. 유한번 쪼개서 만들 수 있다면 오히려 좋음!)
:::
